...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CCN unit input in lnReqAmount, we generate the URI with CCN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CCN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
